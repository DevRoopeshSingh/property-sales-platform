      ...
      <div className="relative h-64 overflow-hidden bg-[var(--color-surface-3)]">
        ...
        {(property.status === 'SOLD' || property.status === 'RENTED') && (
          <div className="absolute top-3 left-3 z-10">
            <span className={cn(
              "px-2.5 py-1 text-xs font-bold uppercase tracking-wider rounded-md shadow-sm",
              property.status === 'SOLD' ? "bg-red-600 text-white" : "bg-purple-600 text-white"
            )}>
              {property.status}
            </span>
          </div>
        ...
        {/* WhatsApp hover button (Only for Active properties) */}
        {property.status === "ACTIVE" && (
          <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10">
            <a
              ...
              className="btn btn-whatsapp text-xs px-3 py-1.5 shadow-md"
